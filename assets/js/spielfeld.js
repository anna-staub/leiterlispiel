class Spielfeld {
  ...
  constructor() {
    for (let i = 99; i>-1; i--) {
      let j=i // i = Platz im Array, j = Feldbeschriftung
      ...
      if (debug_modus) {console.log('konfig ist: '+konfig);} // gibt undefined bei normalem Feld oder [object Object] bei Leiterfeld
      if (debug_modus) {console.log('landekonfig ist: '+landekonfig);} // gibt undefined bei normalem Feld oder [object Object] bei Leiterfeld
      ...
      if (konfig) { 
        ...
        if (debug_modus) {console.log('ist feld leiterfeld? ');}
        if (debug_modus) {console.log(feld instanceof Leiterfeld);}  // gibt true
        if (debug_modus) {console.log('zielfeld: '+feld.Zielfeld);}

        // Leiterfeld der Hoch- oder Runter-Klasse zuordnen
        if (j > konfig.zielfeld) {
          if (debug_modus) {console.log('leiterfeld runter');}
          feld.leiterfeldRunterStartKlassieren();
        } else if (j < konfig.zielfeld) {
          if (debug_modus) {console.log('leiterfeld hoch');}
        ...
        if (debug_modus) {console.log('ist feld leiterfeld? ');}
        if (debug_modus) {console.log(feld instanceof Leiterfeld);} // gibt false
        ...
        if (landekonfig) {
          if (landekonfig.zielfeld < landekonfig.id) {
            if (debug_modus){console.log('Zielfeld is grösser als ID')}
          feld.leiterfeldRunterEndeKlassieren();
        } else if (landekonfig.zielfeld > landekonfig.id){
          if (debug_modus){console.log('Zielfeld is kleiner als ID')}
